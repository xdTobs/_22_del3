...
public class Message {
    ...
    public static Message payRent(String playerName, String fieldName) {
        return of(Type.PAY_RENT, playerName, fieldName);
    }

    public static Message finishBuyingHouses() {
        return of(Type.STOP_BUYING_HOUSES);
    }
    public static Message selectFieldToBuyHouseAt() {
    ...
    public static Message playerName(int id) {
        return switch (id) {
            case (1) -> of(Type.PLAYER_NAME_1);
            case (2) -> of(Type.PLAYER_NAME_2);
            case (3) -> of(Type.PLAYER_NAME_3);
            case (4) -> of(Type.PLAYER_NAME_4);
            default -> throw new IllegalArgumentException("The player doesn't exist.\nIllegal ID: " + id);
        };
    }
